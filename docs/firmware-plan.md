...
- **Variante de l'eFuse (résolu le 26.09.2026)** : le TPS259573 est en **auto-retry** (seuil de surtension réglable, activation active à l'état bas), d'après les pages produit TI citées par une recherche web ; la datasheet elle-même n'a pas pu être ouverte (site TI bloqué par le proxy), à confirmer dans son tableau « Device Comparison », avec le délai d'auto-retry. Conséquence : un défaut persistant fait redémarrer la carte en boucle ; la détection de boucle de redémarrage (§4) arrête cela en passant en mode réduit.
...
| D5 | **eFuse latch-off ou auto-retry** | le firmware gère les deux cas sans connaître la variante — détection de boucle de redémarrage et mode réduit (§4). **26.09.2026 : variante identifiée, auto-retry** (§2.5) : c'est exactement le cas couvert |
